...
{
    private readonly string _remoteServiceBaseUrl = "api/catalog/";


    public Task<CatalogItem?> GetCatalogItem(int id)
    {
        var uri = $"{_remoteServiceBaseUrl}items/{id}?api-version=1.0";
    ...
    {
        var uri = GetAllCatalogItemsUri(_remoteServiceBaseUrl, pageIndex, pageSize, brand, type);
    ...
    {
        var uri = $"{_remoteServiceBaseUrl}items/by?ids={string.Join("&ids=", ids)}&api-version=1.0";
    ...
    {
        var url = $"{_remoteServiceBaseUrl}items/withsemanticrelevance/{HttpUtility.UrlEncode(text)}?pageIndex={page}&pageSize={take}&api-version=1.0";
    ...
    {
        var uri = $"{_remoteServiceBaseUrl}catalogBrands?api-version=1.0";
    ...
    {
        var uri = $"{_remoteServiceBaseUrl}catalogTypes?api-version=1.0";
